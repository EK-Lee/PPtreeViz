...
#' Tree.result
#' plot(Tree.result)
plot.PPtree<-function(x,font.size=17,width.size=1,...){
   #library(grid)
   PPtreeobj <- x
   plotPPtree<-function(PPtreeobj,node.id,xlim,ylim){
      ...
      lf<-ifelse(TS[TS[node.id,2],2]==0,0.5,n.final(PPtreeobj,TS[node.id,2],"right"))  
      rf<-ifelse(TS[TS[node.id,3],2]==0,0.5,n.final(PPtreeobj,TS[node.id,3],"left")) 
   ...
   node.inner.PPtree <- function(PPtreeobj,node.id){   
      ...
      grid.polygon(x = unit(c(xell, rev(xell)), "npc"),
                   ...
                   gp = gpar(fill = "white"))
      grid.text(label.t, y = 0.3,gp=gpar(fontsize=font.size),just=c("center","bottom"))

      Inner.Node.Id.V <- viewport(x = unit(0.5, "npc"), 
                                  y = unit(1, "npc"),
	                                width = max(unit(1, "lines"), 
                                              unit(1.2, "strwidth",as.character(node.id))),
	                                height = max(unit(1, "lines"), 
                                               unit(1.2, "strheight",as.character(node.id))),
   ...
   node.terminal.PPtree<- function(PPtreeobj,node.id){
      ...
      set.unit<-(sum(tolower(temp)!=temp)*0.65+sum(tolower(temp)==temp)*0.5)/gN.width
   ...
   grid.text(y = unit(1, "lines"), "Projection Pursuit Classification Tree", just = "center")
   upViewport()
