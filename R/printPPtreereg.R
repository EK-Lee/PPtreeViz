...
#' @usage print.PPtreereg(x,tree.print=TRUE,coef.print=FALSE,cutoff.print=FALSE,verbose=TRUE,...)
...
print.PPtreereg<-function(x,tree.print=TRUE,coef.print=FALSE,
                          cutoff.print=FALSE,verbose=TRUE,...){
   PPregtreeOBJ<-x
   ...
   MSE<-matrix(c(PPreg.predict(PPregtreeOBJ,sample.data.X,
                               Rule=1,sample.data.Y)$predict.MSE,
                 PPreg.predict(PPregtreeOBJ,sample.data.X,
                               Rule=2,sample.data.Y)$predict.MSE,
                 PPreg.predict(PPregtreeOBJ,sample.data.X,
                               Rule=3,sample.data.Y)$predict.MSE,
                 PPreg.predict(PPregtreeOBJ,sample.data.X,
                               Rule=4,sample.data.Y)$predict.MSE,
                 PPreg.predict(PPregtreeOBJ,sample.data.X,
                               Rule=5,sample.data.Y)$predict.MSE,
                 PPreg.predict(PPregtreeOBJ,sample.data.X,
                               Rule=6,sample.data.Y)$predict.MSE,
                 PPreg.predict(PPregtreeOBJ,sample.data.X,
                               Rule=7,sample.data.Y)$predict.MSE,
                 PPreg.predict(PPregtreeOBJ,sample.data.X,
                               Rule=8,sample.data.Y)$predict.MSE,                 
                 PPreg.predict(PPregtreeOBJ,sample.data.X,
                               Rule=9,sample.data.Y)$predict.MSE),nrow=1)               
